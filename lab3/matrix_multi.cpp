...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        if (MPI_COMM_WORLD == MPI_COMM_NULL || true) {
    ...
    std::vector<double> local_C(n * (row_end - row_start), 0.0);
    ...
    MPI_Scatterv(local_A.data(), sendcounts.data(), displs.data(), MPI_DOUBLE,
    ...
    MPI_Gatherv(local_C.data(), sendcounts[rank], MPI_DOUBLE,
                global_C.data(), recvcounts.data(), recvdispls.data(), MPI_DOUBLE,
                0, MPI_COMM_WORLD);
